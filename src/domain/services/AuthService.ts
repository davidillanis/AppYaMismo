...
export const loginUser = async (payload: {
  email: string;
  password: string;
}): Promise<ResponseStatusDTO<LoginResponseDTO>> => {
  let url = `${BASE_PATH}/login`;
  const response = await apiClient.post<ResponseStatusDTO<LoginResponseDTO>>(
    url,
    payload,
  );
...
export const validateToken = async (
  token: string,
): Promise<ResponseStatusDTO<boolean>> => {
  // Aseguramos que el token no sea undefined o string "null"
  if (!token || token === "null" || token === "undefined") {
    return { isSuccess: false, data: false, message: "Token inválido" };
  }
